...
// Define a ordem física para a pontuação: 0=Amarelo, 1=Vermelho, 2=Verde, 3=Azul
...
{
    audio_stop();
    g_audio_data = WAV_DATA_WIN;
    g_audio_len = WAV_DATA_LENGTH_WIN;
    g_audio_pos = 0;
...
{
    audio_stop();
    g_audio_data = WAV_DATA_GA;
    g_audio_len = WAV_DATA_LENGTH_GA;
    g_audio_pos = 0;
    g_audio_playing = true;
...
static void pwm_interrupt_handler(void)
{
    ...
    {
        uint8_t original_sample = g_audio_data[g_audio_pos >> 3];
        pwm_set_gpio_level(AUDIO_PIN, original_sample / 8);
...
static void show_score_blink(uint8_t score)
{
    ...
    // Etapa 1: Binário usando a ordem física da sua caixa
    for (int i = 0; i < BUTTON_COUNT; i++)
    {
        // Busca qual LED representa este bit (1, 2, 4 ou 8)
        int idx_led = ORDEM_PONTUACAO[i];
        gpio_put(LEDS[idx_led], (score >> i) & 1);
    }
    sleep_ms(3000);
    all_leds_off();
    sleep_ms(400);

    // Etapa 2: Pisca a quantidade de pontos em sequência física
...
static void victory_fx_with_music(void)
{
    int step = 0;
    for (int rep = 0; rep < 6; rep++)
    {
        ...
        {
            all_leds_off();
            gpio_put(LEDS[step % BUTTON_COUNT], 1);
            step++;
            sleep_ms(100);
        }
    }
